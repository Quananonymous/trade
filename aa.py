...
class TradingFutureBot:
    def __init__(self, coin, leverage, percent_balance, tp, sl, direction):
        self.coin = coin
        self.leverage = leverage
        self.percent_balance = percent_balance
        self.tp = tp
        self.sl = sl
        self.direction = direction  # "long" hoặc "short"
        ...
        while self.status:
            try:
                if not self.check_position_open(account):
                    result = self.place_order(account)
                    if update_callback:
                        update_callback(f"{self.coin.get_name()}: {result}")
                time.sleep(5)
    ...
    def place_order(self, account):
        ...
        # Đặt lệnh market
        side = SIDE_BUY if self.direction == "long" else SIDE_SELL
        ...
        # Đặt TP/SL nếu có
        if self.tp > 0:
            tp_price = price * (1 + self.tp / 100) if self.direction == "long" else price * (1 - self.tp / 100)
            account.client.futures_create_order(
                symbol=self.coin.get_name(),
                side=SIDE_SELL if self.direction == "long" else SIDE_BUY,
                type=ORDER_TYPE_TAKE_PROFIT_MARKET,
                stopPrice=round(tp_price, 2),
                closePosition=True
            )
        if self.sl > 0:
            sl_price = price * (1 - self.sl / 100) if self.direction == "long" else price * (1 + self.sl / 100)
            account.client.futures_create_order(
                symbol=self.coin.get_name(),
                side=SIDE_SELL if self.direction == "long" else SIDE_BUY,
        ...
        return f"Đã mở vị thế {self.direction.upper()} {self.coin.get_name()} với {qty} coin!"

# Giao diện GUI với Tkinter, hỗ trợ nhiều vị thế và tự động mở lại
class FutureBotGUI:
    def __init__(self, root):
        ...
        # Giao diện đặt lệnh
        self.frame_order = ttk.LabelFrame(root, text="Đặt lệnh Future")
        ...
        ttk.Label(self.frame_order, text="Kiểu:").grid(row=5, column=0, sticky="w")
    ...
    def place_order(self):
        ...
        try:
            ...
            bot = TradingFutureBot(coin, leverage, percent, tp, sl, direction)
            self.bots[symbol] = bot
            self.status_list.insert(tk.END, f"{symbol} | {direction.upper()} | Đòn bẩy: {leverage} | %Số dư: {percent}")
